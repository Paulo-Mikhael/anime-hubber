...
// Returns a masked div in a specified position on carousel
function MaskedImage(position: ImagePositions) {
  return ({ imageUrl }: { imageUrl: string | undefined }) => {
...
// Image on carousel's center
function CenterImage({ centerPostImage }: { centerPostImage: IPost }) {
  return (
    <span className="flex flex-col items-start gap-6 z-10">
      <div className="relative flex items-end">
        <div className="w-[559px] h-[389px]">
          <Image fill alt={centerPostImage.postImage.alt} src={centerPostImage.postImage.url} className="rounded-[28px] static" />
        </div>
        <span className="absolute m-6 left-0 right-0 flex flex-col gap-2.5 z-2">
          ...
          <span className="h-24 overflow-scroll scrollbar flex flex-col">
            <p className="text-paragraph">{centerPostImage.title}</p>
            <p className="text-description">
              {centerPostImage.content}
            </p>
          </span>
        </span>
        <span className="absolute bg-black/80 inset-0 rounded-[28px] border-[3px] border-primary-1000" />
      </div>
      <Tag>
        <Tag.Element>
          <TagIcon />
        </Tag.Element>
        {centerPostImage.tags.map((tag) => (
          <Tag.Element key={uuid()}>
            <p>{tag}</p>
          </Tag.Element>
        ))}
      </Tag>
